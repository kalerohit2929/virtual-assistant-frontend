...
function SignUp() {
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(ellipse at 20% 50%, #0d0d2b 0%, #000008 60%, #050515 100%)',
      ...
      {/* Glowing orbs */}
      <div style={{ position: 'absolute', top: '-10%', right: '-5%', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,80,255,0.12) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '-10%', left: '-5%', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,200,255,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <style>{`
        @keyframes gridMove { from { backgroundPosition: 0 0; } to { backgroundPosition: 60px 60px; } }
        @keyframes pulse { 0%,100% { opacity:0.6; } 50% { opacity:1; } }
        @keyframes slideIn { from { opacity:0; transform:translateY(30px); } to { opacity:1; transform:translateY(0); } }
        @keyframes borderGlow { 0%,100% { border-color: rgba(0,120,255,0.4); box-shadow: 0 0 0 0 rgba(0,120,255,0); } 50% { border-color: rgba(0,180,255,0.8); box-shadow: 0 0 20px rgba(0,120,255,0.3); } }
        .va-input { width:100%; height:56px; outline:none; border:1.5px solid rgba(0,120,255,0.35); background:rgba(0,20,60,0.4); color:#e0eeff; padding: 0 48px 0 20px; border-radius:4px; font-size:16px; font-family:'Rajdhani',sans-serif; letter-spacing:0.5px; transition:all 0.3s; }
        .va-input::placeholder { color:rgba(150,180,220,0.5); }
        .va-input:focus { border-color:rgba(0,160,255,0.8); background:rgba(0,30,80,0.5); box-shadow: 0 0 20px rgba(0,120,255,0.2), inset 0 0 20px rgba(0,50,150,0.1); }
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: '11px', letterSpacing: '4px', color: '#0080ff', marginBottom: '10px', opacity: 0.8 }}>SYSTEM INITIALIZE</div>
          <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: '26px', fontWeight: 900, color: '#fff', margin: 0, letterSpacing: '1px' }}>
            CREATE <span style={{ color: '#00aaff' }}>ACCOUNT</span>
          </h1>
        ...
        <form onSubmit={handleSignUp} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ position: 'relative' }}>
            <input type="text" placeholder="Full Name" className="va-input" required value={name} onChange={e => setName(e.target.value)} />
          ...
          <div style={{ position: 'relative' }}>
            <input type={showPassword ? "text" : "password"} placeholder="Password" className="va-input" required value={password} onChange={e => setPassword(e.target.value)} style={{ paddingRight: '52px' }} />
          ...
          {err && <div style={{ padding: '10px 16px', background: 'rgba(255,50,50,0.1)', border: '1px solid rgba(255,50,50,0.3)', borderRadius: '4px', color: '#ff6060', fontSize: '14px', letterSpacing: '0.5px' }}>⚠ {err}</div>}

          <button type="submit" disabled={loading} className="va-btn" style={{
            marginTop: '8px', height: '54px', borderRadius: '4px',
            background: 'linear-gradient(135deg, #0040cc, #0080ff)',
            color: '#fff',
          }}>
        ...
        <div style={{ textAlign: 'center', marginTop: '24px', color: 'rgba(150,180,220,0.6)', fontSize: '14px', letterSpacing: '0.5px' }}>
          Already have access?{" "}
          <span onClick={() => navigate("/signin")} style={{ color: '#00aaff', cursor: 'pointer', fontWeight: 600, textDecoration: 'none' }}>
            SIGN IN →
          </span>
        </div>
      </div>
